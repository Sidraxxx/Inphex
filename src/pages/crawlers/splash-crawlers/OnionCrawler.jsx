...
import { Sun, Moon, Calendar, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Eye, EyeOff, Activity, TrendingUp, TrendingDown, Clock, Database } from 'lucide-react'
...
{ name: "Day 1", green: 2.5, red: 1.8 },
{ name: "Day 2", green: 2.0, red: 1.5 },
{ name: "Day 3", green: 2.3, red: 1.7 },
{ name: "Day 4", green: 1.8, red: 1.2 },
{ name: "Day 5", green: 2.6, red: 1.9 },
{ name: "Day 6", green: 2.4, red: 1.6 },
]
...
<div className="flex items-center justify-between py-2">
  ...
  <button
    ...
    } ${darkMode && !checked ? "bg-gray-600 hover:bg-gray-500" : ""}`}
  >
    <span
...
return (
  <div className="absolute top-full left-0 mt-2 z-50" ref={containerRef}>
    ...
    >
      ...
      <div className={`px-4 py-3 border-b ${darkMode ? "border-gray-600" : "border-gray-200"}`}>
        <div className="flex items-center justify-between">
          <button
            ...
            className={`p-1 rounded-md hover:bg-gray-100 ${darkMode ? "hover:bg-gray-700" : ""} transition-colors`}
          >
      ...
      {/* Footer */}
      <div
        className={`px-4 py-3 border-t ${darkMode ? "border-gray-600" : "border-gray-200"} flex justify-end space-x-2`}
      >
        <button
          onClick={onClose}
...
>
  {(header || title) && (
    <div className={`px-6 py-4 border-b ${darkMode ? "border-gray-700 bg-[#232A34]/70" : "border-gray-200"}`}>
      {header || <h3 className="text-sm font-semibold">{title}</h3>}
    </div>
  )}
  {children}
</div>
...
// Status indicator component (Rewritten)
const StatusIndicator = ({ label, count, bgColor }) => (
<span className={`inline-flex items-center px-3 py-1 rounded-sm text-sm font-medium text-white shadow-md ${bgColor}`}>
  <span className="font-bold px-1">{count}</span> {label}
</span>
)
...
<div className={` ${themeClasses} mt-12`}>
  ...
  <div className=" mx-auto p-6 space-y-8 bg-[#232A34]/10">
      ...
      header={
        ...
        darkMode ? "" : ""
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-sm ">Search Data Range</h2>
      ...
      <div className={`p-6 space-y-4 ${
        ...
        <div className="grid grid-cols-1  gap-4 w-full">
          ...
          <div className=" space-y-4">
            <div className="flex items-center space-x-4"> {/* Changed to flexbox */}
                ...
                <div className="relative" ref={startCalendarRef}>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full pl-10 pr-4 py-3 rounded-lg border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                        darkMode
              ...
              <span className="text-gray-500 dark:text-gray-400 text-2xl font-bold">-</span> {/* Separator */}
              <div className="flex-1 space-y-2">
          ...
          {/* Domain Toggles */}
          <div
            className={`w-full p-4 rounded-lg border-1 border-dashed  ${
              darkMode ? "border-gray-600" : "border-gray-300 bg-gray-50"
            }`}
          >
        ...
        {/* Enhanced Status Indicators (Updated to match image) */}
        <div className="flex flex-wrap gap-4 justify-end"> {/* Changed to flex-wrap and justify-end */}
          <StatusIndicator
            label="UP"
            count={domainUp ? upCount : 0}
            bgColor="bg-[#34543B]" // Dark green
          />
          <StatusIndicator
            label="DOWN"
            count={domainDown ? downCount : 0}
            bgColor="bg-[#6B2A3A]" // Dark red/maroon
          />
          <StatusIndicator label="CRAWLED" count={crawledCount} bgColor="bg-[#6B5A3A]" /> {/* Dark brown/olive */}
          <StatusIndicator label="QUEUED" count={queuedCount} bgColor="bg-[#2A6B6B]" /> {/* Dark teal/cyan */}
        </div>
      </div>
    </Card>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 ">
      <div> {/* New wrapper div */}
        {/* Enhanced Data Table */}
        <Card className="text-sm" title="Recent Activity">
          <div className="grid grid-cols-[1fr_1fr] gap-4  text-[#50B8E4] dark:text-gray-400 px-4 py-2 border-b">
            <div className="flex items-center space-x-1">
              <span>Date</span>
              <TrendingDown className="w-3 h-3" /> {/* Down arrow icon */}
            </div>
            <div>Item</div>
          </div>
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {getCurrentPageData().map((item, index) => (
              <div
                key={item.id}
                className="flex items-center justify-between p-4 hover:bg-gray-600 dark:hover:bg-gray-700/50 transition-colors"
              >
                <div className="flex items-center space-x-3">
                  <div
          ...
          {/* Enhanced Pagination */}
          <div
            className={`px-4 py-4 border-t ${darkMode ? "border-gray-700 bg-gray-800/50" : "border-gray-200 bg-gray-50"}`}
          >
            <div className="flex items-center justify-between">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={statisticsData}>
                  ...
                  <YAxis
                    ...
                    tickLine={false}
                    tick={{ fontSize: 12, fill: darkMode ? "#9CA3AF" : "#6B7280" }}
                    domain={[0, 3]} // Adjusted domain
                    ticks={[0, 0.5, 1, 1.5, 2, 2.5, 3]} // Adjusted ticks
                  />
                  <Bar dataKey="green" fill="#10B981" radius={[3, 3, 0, 0]} maxBarSize={30} /> {/* Green bar */}
                  <Bar dataKey="red" fill="#F56C89" radius={[3, 3, 0, 0]} maxBarSize={30} /> {/* Pink/Red bar */}
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center space-x-3">
        ...
        <h3 className="text-md font-bold">Historical Analytics</h3>
      </div>
